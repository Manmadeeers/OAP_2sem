...
void sumLevels(Node* node, vector<int>& sum, int level);
...
void main()
{
    ...
    do {
        ...
        case 5:FreeTree(root); cout << "The tree is deleted" << endl;
            break;
        case 6:cout << "Tree height is:  "<<tree_height(root) << endl;
            sumLevels(root, sum, 0);
            for (int i = 0; i < levels; i++) {
                cout << "Sum of keys at level " << i << ": " << sum[i] << endl;
            }
            break;
        }
...
int tree_height(Node* node)//function to calculate the max height of a tree
{
    if (node == nullptr) {
        return 0;
    }
...
void sumLevels(Node* node, vector<int>& sum, int level) {
    ...
    sum[level] += node->key; // Add the current node's key to the sum at its level
    sumLevels(node->left, sum, level ); // Recursively call for left subtree
    sumLevels(node->right, sum, level ); // Recursively call for right subtree
}