...
void Calculate_and_count(Node* root, double& sum, double& count);
double calculate_average(Node* root);
...
void main()
{
    ...
    do {
        ...
        cout << "6-Calculate the average value" << endl;
        cout << "7-Exit" << endl;
        cin >> choise;
        ...
        case 5:FreeTree(root); cout << "The tree is deleted" << endl;
            break;
        case 6:cout << "The average value is: " << calculate_average(root) << endl;
            break;
        }
...
void Calculate_and_count(Node* root, double& sum, double& count)//function to count the sum of all the elements and count the elements themselves
{
    if (root == nullptr) {//if the tree is empty
        return;
    }
    sum += root->key;
    count++;
    Calculate_and_count(root->right, sum, count);//recursion to count elements in all the branches
    Calculate_and_count(root->left, sum, count);
}
double calculate_average(Node* root)//function to calculate the average value
{
    double average;//var for average value
    double sum = 0;
    double count = 0;
    Calculate_and_count(root, sum, count);//use previously written function
